...
class WebSocketHandler(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, message):
        ...
        if not ws_id:
            ...
            if not bot_id:
                print(f"disconnecting: missing user id {self.request.remote_ip}\nData: {data}")
                ...
                if self.auth_enabled:
                    ...
                    if users_ws[data["user"]].token != token:
                        ...
                        data.update(
                            {
                                "op": "exception",
                                "message": "Invalid token! If in doubt, generate a new token using the bot command: /rich_presence."
                            }
                        )
        ...
        if len(token) not in (0, 50):
            self.write_message(
                json.dumps({"op": "disconnect", "reason": f"The token must be 50 characters..."}))
            self.close(code=4200)
            return
...
class WSClient:
    ...
    async def connect(self):
        ...
        print("🌐 - RPC client connected, syncing bots rpc...")
    ...
    async def connect_bot_rpc(self):
        ...
        if not bot_ids:
            print("🌐 - RPC server connection ignored: Bot list is empty...")
            return
        ...
        while True:
            ...
            try:
                ...
                if isinstance(e, aiohttp.WSServerHandshakeError):
                    print(f"🌐 - Failed to connect to RPC server, retrying in {(b:=int(self.backoff))} second{'s'[:b^1]}.")
                else:
                    print(f"🌐 - RPC server connection lost - Reconnecting in {(b:=int(self.backoff))} second{'s'[:b^1]}.")
