            ...
            if current_progress != previows_progress:
                ...
                try:
                    ...
                    if download_speed >= 1:
                        ...
                        speed_txt = "KB/s"
                    print(f"Downloading {filename} {current_progress}% complete ({download_speed:.2f} {speed_txt} / {total_txt})")
                except:
                    print(f"Downloading {filename} {current_progress}% complete")
...
def run_lavalink(
    ...
    print("🌋 - Starting the Lavalink server (depending on the hosting, lavalink may take a while to start, "
          "which may cause some connection failures until it is fully initialized).")

    lavalink_process = subprocess.Popen(java_cmd.split(), stdout=subprocess.DEVNULL)

    if lavalink_additional_sleep:
        print(f"🕙 - Please wait {lavalink_additional_sleep} seconds...")
