...
def can_send_message(
    ...
    if not channel.permissions_for(channel.guild.me).embed_links:
        raise GenericError(f"**{bot.mention} does not have permission to embed links in the channel: {channel.mention}**")
...
async def check_requester_channel(ctx: CustomContext):
    ...
    error_msg = "**At this moment you can only use slash commands (/) in this channel!**"
...
async def check_pool_bots(inter, only_voiced: bool = False, check_player: bool = True, return_first=False,
    ...
    if not bot_in_guild:
        ...
        msg = "**There are no compatible music bots in the server...**"
        ...
        if bot_missing_perms:
            msg = f"**There are music bots available in the server but they lack permission to send messages in the channel <#{inter.channel_id}>**:\n\n" + \
        ...
        if extra_bots_counter:
            if inter.author.guild_permissions.manage_guild:
                ...
                msg += "\n\n**Or, if you prefer, you can ask an administrator/manager of the server to click the button below " \
...
def can_send_message_check():
    ...
    async def predicate(inter):
        ...
        if not inter.guild:
            ...
            raise GenericError("**This command must be used in a server...**")
...
def check_queue_loading():
    ...
    async def predicate(inter):
        ...
        if player.locked:
            raise GenericError("**It is not possible to perform this action while the song is being processed "
...
def check_stage_topic():
    ...
    async def predicate(inter):
        ...
        if player.stage_title_event and (time_:=int((disnake.utils.utcnow() - player.start_time).total_seconds())) < time_limit and not (await bot.is_owner(inter.author)):
            raise GenericError(
                f"**You will need to wait {time_format((time_limit - time_) * 1000, use_names=True)} to use this feature "
        ...
        if player.current and player.current.info["sourceName"] == "youtube" and (remaining:=(disnake.utils.utcnow() - player.start_time).total_seconds()) < bot.config["YOUTUBE_TRACK_COOLDOWN"]:
            if not await bot.is_owner(inter.author):
                raise GenericError("**{}, you can only skip the current YouTube song in {}**.\n"
                                   "-# This is a way to help avoid possible YouTube blocks on song playback".format(inter.author.mention, time_format((bot.config["YOUTUBE_TRACK_COOLDOWN"] - int(remaining))*1000, use_names=True)))
...
def get_available_bots_info(pool: BotPool, guild_id: int, member: disnake.Member):
    ...
    if available_bots:
        txts.append(f"You can use another{(s:='s'[:(abcount:=len(available_bots))^1])} available music bot{s} in the server to use in another voice channel: " + " ".join(available_bots))
    else:
        t = ""
        if voice_channels:
            t += f"You can join one of the channels with active sessions in the server: " + " ".join(voice_channels)
...
async def check_player_perm(inter, bot: BotCore, channel, guild_data: dict = None):
    ...
    if player.keep_connected:
        ...
        raise GenericError("Only members with the **manage channels** permission "
                           f"can use this command/button with **24/7 mode active** in the channel <#{player.channel_id}>...\n\n" + txt, components=components)
    ...
    if not vc and inter.author.voice:
        ...
        await channel.send(embed=disnake.Embed(
            description=f"{inter.author.mention} was added to the DJ list because there was no DJ in the channel <#{vc.id}>.",
...
def can_connect(
    ...
    if not perms.connect:
        raise GenericError(f"**I don't have permission to connect to the channel {channel.mention}**")

    if not isinstance(channel, disnake.StageChannel):

        if not perms.speak:
            raise GenericError(f"**I don't have permission to speak in the channel {channel.mention}**")
    ...
    if bot:
        for b in bot.pool.get_guild_bots(channel.guild.id):
            ...
            if b.bot_ready and b.user.id in channel.voice_states:
                raise PoolException()
                #raise GenericError(f"**There is already a bot connected in the channel {channel.mention}\n"
