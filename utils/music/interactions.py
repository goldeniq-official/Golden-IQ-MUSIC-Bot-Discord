...
class VolumeInteraction(disnake.ui.View):
    ...
    def process_buttons(self):
        ...
        for l in [5, 20, 40, 60, 80, 100, 120, 150]:
            ...
            if l > 100:
                description = "Above 100% the audio may sound very bad."
            else:
                description = None
...
class QueueInteraction(disnake.ui.View):
    ...
    def update_embed(self):
        ...
        self.embed.title = f"**Queue songs [Page: {self.current_page+1} / {self.max_page+1}]**"
    ...
    async def track_select_callback(self, interaction: disnake.MessageInteraction):
        ...
        if not track:
            await interaction.send(f"Song with id \"{track_id}\" not found in the player queue...", ephemeral=True)
            return
    ...
    async def invoke_command(self, interaction: disnake.MessageInteraction):
        ...
        try:
            ...
            await interaction.send("The player has already been stopped...", ephemeral=True)
            self.stop()
            return
    ...
    async def interaction_check(self, interaction: disnake.MessageInteraction):
        ...
        if interaction.author != self.user:
            await interaction.send(f"Only the member {self.user.mention} can interact here.", ephemeral=True)
            return

        try:
            self.bot.music.players[self.user.guild.id]
        except KeyError:
            await interaction.response.edit_message(content="The player has been stopped...", embed=None, view=None)
            self.stop()
            return
...
class FavModalImport(disnake.ui.Modal):
    ...
    def __init__(self, view):
        ...
        if self.view.mode == ViewMode.guild_fav_manager:
            super().__init__(
                title="Import Playlists to Server",
                components=[
                    disnake.ui.TextInput(
                        style=disnake.TextInputStyle.long,
                        label="Insert data (in json format)",
    ...
    async def callback(self, inter: disnake.ModalInteraction, /) -> None:
        ...
        try:
            ...
            await inter.send("**An error occurred while parsing the data or invalid/unformatted "
                               f"json data was submitted.**\n\n`{repr(e)}`", ephemeral=True)
            return
        ...
        if self.view.mode == ViewMode.fav_manager:
            ...
            if retry_after := self.view.bot.get_cog("Music").fav_import_export_cd.get_bucket(inter).update_rate_limit():
                if retry_after < 1:
                    retry_after = 1
                await inter.send("**You must wait {} to import.**".format(
            ...
            for name, url in json_data.items():
                ...
                if len(url) > (max_url_chars := self.view.bot.config["USER_FAV_MAX_URL_LENGTH"]):
                    await inter.send(
                        f"**An item in your file {url} exceeds the allowed character limit: {max_url_chars}**",
                        ephemeral=True)
                    return
            ...
            for name in json_data.keys():
                if len(name) > (max_name_chars := self.view.bot.config["USER_FAV_MAX_NAME_LENGTH"]):
                    await inter.edit_original_message(
                        f"**An item in your file ({name}) exceeds the allowed character limit: {max_name_chars}**")
                    return
                try:
            ...
            if self.view.bot.config["MAX_USER_FAVS"] > 0 and not (await self.view.bot.is_owner(inter.author)):
                ...
                if (json_size := len(json_data)) > self.view.bot.config["MAX_USER_FAVS"]:
                    await inter.edit_original_message(f"The number of items in your favorites file exceeds "
                                                      f"the maximum allowed ({self.view.bot.config['MAX_USER_FAVS']}).")
                    return
            ...
            await inter.edit_original_message(content="**Favorites imported successfully!**")

            if (s := len(json_data)) > 1:
                self.view.log = f"{s} favorites were imported successfully."
            else:
                name = next(iter(json_data))
                self.view.log = f"The favorite [`{name}`]({json_data[name]}) was imported successfully."
            ...
            for name, data in json_data.items():
                ...
                if len(data['url']) > (max_url_chars := self.view.bot.config["USER_FAV_MAX_URL_LENGTH"]):
                    await inter.send(
                        f"**An item in your file exceeds the allowed character limit: {max_url_chars}\nURL:** {data['url']}",
                        ephemeral=True)
                    return
            ...
            if not self.view.guild_data["player_controller"]["channel"] or not self.view.bot.get_channel(
                ...
                await inter.edit_original_message("**No player configured in the server! Use the /setup command**")
                return
            ...
            if (json_size := len(json_data)) > 25:
                await inter.edit_original_message(
                    f"The number of items in the file exceeds the maximum allowed (25).")
                return

            if (json_size + (user_favs := len(self.view.guild_data["player_controller"]["fav_links"]))) > 25:
                await inter.edit_original_message(
                    "The server's song/playlist list doesn't have enough space to add all items from your file...\n"
                    f"Current limit: 25\n"
                    f"Number of saved links: {user_favs}\n"
            ...
            await inter.edit_original_message(content="**Server fixed links were imported successfully!**")

            if (s := len(json_data)) > 1:
                self.view.log = f"{s} links were imported successfully to the server favorites list."
            else:
                name = next(iter(json_data))
                self.view.log = f"The link [`{name}`]({json_data[name]}) was imported successfully to the server links list."
            ...
            if self.view.bot.config["MAX_USER_INTEGRATIONS"] > 0 and not (await self.view.bot.is_owner(inter.author)):
                ...
                if (json_size := len(json_data)) > self.view.bot.config["MAX_USER_INTEGRATIONS"]:
                    await inter.edit_original_message(f"The number of items in your integrations file exceeds "
                                       f"the maximum allowed ({self.view.bot.config['MAX_USER_INTEGRATIONS']}).")
                    return

                if (json_size + (user_integrations := len(self.view.data["integration_links"]))) > self.view.bot.config[
                    "MAX_USER_INTEGRATIONS"]:
                    await inter.edit_original_message(
                        "You don't have enough space to add all the integrations from your file...\n"
            ...
            if (s := len(json_data)) > 1:
                self.view.log = f"{s} integrations were imported successfully."
            else:
                name = next(iter(json_data))
                self.view.log = f"The integration [`{name}`]({json_data[name]}) was imported successfully."
        ...
        if self.view.mode == ViewMode.fav_manager:
            ...
            try:
                ...
                if len(self.view.data["fav_links"]) >= self.view.bot.config["MAX_USER_FAVS"]:
                    await inter.edit_original_message(
                        "**There is no available space to add new favorites (remove one and try again).**")
                    return
            ...
            await inter.edit_original_message(
                embed=disnake.Embed(
                    description="**Link saved/updated successfully in your favorites!\n"
                                "It will appear in the following situations:** ```\n"
                                "- When using the /play command (selecting in the search autocomplete)\n"
                                "- When clicking the play favorites button in the player.\n"
                                "- When using the play (prefixed) command without a name or link.```",
            ...
            try:
                ...
                if len(self.view.guild_data["player_controller"]["fav_links"]) > 24:
                    await inter.edit_original_message("**There is no available space to add new favorites (remove one and try again).**")
                    return
            ...
            await inter.edit_original_message(
                embed=disnake.Embed(description="**Link added/updated successfully in the player fixed links!\n"
                                                "Members can use it directly in the player-controller when it's not in use.**",
            ...
            if len(self.view.data["integration_links"]) >= self.view.bot.config["MAX_USER_FAVS"]:
                await inter.edit_original_message(
                    "**There is no available space to add new integrations (remove one and try again).**")
                return
            ...
            if (matches := spotify_regex_w_user.match(url)):
                ...
                if not self.view.bot.spotify:
                    await inter.edit_original_message(
                        embed=disnake.Embed(
                            description="**Spotify support is not available at the moment...**",
                            color=disnake.Color.red()
                        )
                    )
                    return
                ...
                if url_type != "user":
                    await inter.edit_original_message(
                        embed=disnake.Embed(
                            description=f"**You must use a link from a Spotify user profile.** {url}",
                            color=disnake.Color.red()
                        )
                    )
                    return
                ...
                try:
                    ...
                    await inter.edit_original_message(
                        embed=disnake.Embed(
                            description="**An error occurred while getting Spotify information:** ```py\n"
                                        f"{repr(e)}```",
                ...
                if not result:
                    await inter.edit_original_message(
                        embed=disnake.Embed(
                            description="**The user from the provided link has no public playlists...**",
                            color=self.view.bot.get_color()
                        )
                    )
                    return
                ...
                if url_type != "profile":
                    await inter.edit_original_message(
                        embed=disnake.Embed(
                            description=f"**You must use a link from a Deezer user profile.** {url}",
                            color=disnake.Color.red()
                        )
                    )
                    return
                ...
                try:
                    ...
                    await inter.edit_original_message(
                        embed=disnake.Embed(
                            description="**An error occurred while getting Deezer information:** ```py\n"
                                        f"{repr(e)}```",
                            color=self.view.bot.get_color()
                        )
                    )
                ...
                if match:
                    ...
                    if match:
                        ...
                        await inter.edit_original_message(
                            embed=disnake.Embed(
                                description=f"**Provided link is not supported:** {url}",
                                color=disnake.Color.red()
                            )
                        )
                        return
                ...
                try:
                    ...
                    traceback.print_exc()
                    await inter.edit_original_message(f"**An error occurred while fetching the URL information:** ```py\n{repr(e)}```")
                    return
                ...
                if not info:
                    ...
                    if source == "[YT]:":
                        msg += f"\n\n`Note: Make sure the link contains a user with @, e.g.: @ytchannel`"
                ...
                if not info['entries']:
                    await inter.edit_original_message(
                        embed=disnake.Embed(
                            description=f"**The user/channel from the provided link has no public playlists...**",
                            color=disnake.Color.red()
                        )
                    )
                    return
            ...
            await inter.edit_original_message(
                embed=disnake.Embed(
                    description=f"**Integration added/edited successfully:** [`{title}`]({data['url']})\n"
                                "**It will appear in the following situations:** ```\n"
                                "- When using the /play command (selecting the integration in the search autocomplete)\n"
                                "- When clicking the play favorites button in the player.\n"
                                "- When using the play (prefixed) command without a name or link.```",
                    color=self.view.bot.get_color(me)
                ), view=None
            ...
            self.view.log = f"[`{data['title']}`](<{data['url']}>) was added to your integrations."
...
class FavMenuView(disnake.ui.View):
    ...
    def update_components(self):
        ...
        if self.mode == ViewMode.fav_manager:
            ...
            if self.data["integration_links"]:
                play_button = disnake.ui.Button(label="Play a playlist from the selected integration", emoji="▶", custom_id="favmanager_play_button")
                play_button.callback = self.play_callback
                self.add_item(play_button)
    ...
    def build_txt(self):
        ...
        if self.mode == ViewMode.fav_manager:
            ...
            if not self.light_mode:
                txt += "\n\n**How to use them?**\n" \
                       f"* Using the {cmd} command (selecting the favorite in the search autocomplete)\n" \
                        "* Clicking the play favorite/integration button/select in the player.\n" \
                        f"* Using the {self.prefix}{self.bot.get_cog('Music').play_legacy.name} command without a song/video name or link.\n" \
            ...
            if not self.guild_data["player_controller"]["fav_links"]:
                txt += f"No links added to bot {self.bot.user.mention} (click the add button below).\n"
                ...
                txt += f"**Current links in bot {self.bot.user.mention}:**\n" + paginator("\n".join(
                    f"> {format_gfav(n+1, d)}" for n, d in enumerate(islice(self.guild_data["player_controller"]["fav_links"].items(), 22))
                ), max_size=1400)[0]

                txt += "\n\n**How to use them?**\n" \
                        f"* Using the player selection menu during idle mode.\n" \
                       f"\n`Selected bot:` {self.bot.user.mention}"

        elif self.mode == ViewMode.integrations_manager:

            txt = "### Integrations Manager for channels/profiles with public playlists.\n"

            if not self.data["integration_links"]:
                txt += "**You have no integrations at the moment (click the add button below).**\n"
    ...
    async def edit_callback(self, inter: disnake.MessageInteraction):
        ...
        if self.mode == ViewMode.fav_manager:
            try:
                ...
                await inter.send(f"**No favorite found with the name:** {self.current}", ephemeral=True)
    ...
    async def remove_callback(self, inter: disnake.MessageInteraction):
        ...
        if self.mode == ViewMode.fav_manager:
            try:
                ...
                await inter.edit_original_message(f"**No favorite found in the list with the name:** {self.current}")
                return

            await self.bot.update_global_data(inter.author.id, self.data, db_name=DBModel.users)

            self.log = f"Favorite {url} was removed successfully!"
            ...
            try:
                ...
                await inter.edit_original_message(
                    embed=disnake.Embed(
                        description=f"**No links found in the list with the name:** {self.current}",
            ...
            self.log = f"Link {url} was removed successfully from the server favorites list!"
            ...
            try:
                ...
                await inter.send(f"**No integration found in the list with the name:** {self.current}", ephemeral=True)
                return

            await self.bot.update_global_data(inter.author.id, self.data, db_name=DBModel.users)

            self.log = f"Integration {url} was removed successfully!"
    ...
    async def clear_callback(self, inter: disnake.MessageInteraction):
        ...
        if self.mode == ViewMode.fav_manager:
            if not self.data["fav_links"]:
                await inter.send("**You have no favorite links!**", ephemeral=True)
                return
            ...
            self.log = "Your favorites list was cleared successfully!"

            await inter.send("### Your favorites were deleted successfully!\n"
                             "`A backup file was generated. If you want to revert this deletion, copy the "
                             "file contents and click the \"import\" button and paste the content in the indicated field.`",
            ...
            self.log = "Server favorites list was cleared successfully!"

            await inter.send("### The server's favorite links were deleted successfully!\n"
                             "`A backup file was generated. If you want to revert this deletion, copy the "
                             "file contents and click the \"import\" button and paste the content in the indicated field.`",
            ...
            if not self.data["integration_links"]:
                await inter.response.edit_message(content="**You have no saved integrations!**", view=None)
                return
            ...
            self.log = "Your integrations list was cleared successfully!"

            await inter.send("### Your integrations were deleted successfully!\n"
                             "`A backup file was generated. If you want to revert this deletion, copy the "
                             "file contents and click the \"import\" button and paste the content in the indicated field.`",
    ...
    async def export_callback(self, inter: disnake.MessageInteraction):
        ...
        if self.mode == ViewMode.fav_manager:
            if not self.data["fav_links"]:
                await inter.send(f"**You have no favorite links..\n"
                                 f"You can add using the command: {cmd}**", ephemeral=True)
                return

            fp = BytesIO(bytes(json.dumps(self.data["fav_links"], indent=4), 'utf-8'))

            await inter.send(embed=disnake.Embed(
                description=f"Your favorites are here.\nYou can import using the command: {cmd}",
                color=self.bot.get_color()), file=disnake.File(fp=fp, filename="favorites.json"), ephemeral=True)

        elif self.mode == ViewMode.guild_fav_manager:
            if not self.guild_data["player_controller"]["fav_links"]:
                await inter.edit_original_message(content=f"**There are no fixed songs/playlists in the server..\n"
                                                          f"You can add using the command: {cmd}**")
            ...
            embed = disnake.Embed(
                description=f"**The server's fixed music/playlist link data is here.\n"
                            f"You can import using the command:** {cmd}",
                color=self.bot.get_color(guild.me))
            ...
            if not self.data["integration_links"]:
                await inter.edit_original_message(f"**You have no integrations added...\n"
                                                  f"You can add using the command: {cmd}**")
                return
    ...
    async def interaction_check(self, inter: disnake.MessageInteraction) -> bool:
        ...
        await inter.send(f"Only the member {self.ctx.author.mention} can interact in this message.", ephemeral=True)
...
class SkinSettingsButton(disnake.ui.View):
    ...
    def update_components(self):
        ...
        select_mode = disnake.ui.Select(
            min_values=1, max_values=1, options=[
                disnake.SelectOption(label="Normal Mode", description="Apply skin to the normal player mode",
                                     value="custom_skins", default=self.mode == "custom_skins"),
                disnake.SelectOption(label="Song-Request", description="Apply skin in the song-request player mode",
    ...
    async def interaction_check(self, inter: disnake.MessageInteraction) -> bool:
        ...
        if inter.user.id != self.user.id:
            await inter.send(f"Only the member {self.user.mention} can use the buttons in this message.", ephemeral=True)
            return False
...
class SetStageTitle(disnake.ui.View):
    ...
    placeholder_text = "```ansi\n[34;1m{track.title}[0m -> Song name\n" \
               "[34;1m{track.author}[0m -> Artist/Uploader/Author name of the song.\n" \
               "[34;1m{track.duration}[0m -> Song duration.\n" \
               "[34;1m{track.timestamp}[0m -> Song duration in countdown (voice channel only).\n" \
               "[34;1m{track.source}[0m -> Song source/origin (Youtube/Spotify/Soundcloud etc)\n" \
               "[34;1m{track.emoji}[0m -> Music source emoji (voice channel only).\n" \
               "[34;1m{track.playlist}[0m -> Name of the source playlist (if any)\n" \
               ...
               "`Note: In the voice channel you can use custom emojis in the status message (including emojis from servers I'm not in and from servers you're not in).`"
    ...
    def __init__(self, ctx: Union[CustomContext, disnake.Interaction], bot: BotCore, guild: disnake.Guild, data: dict):
        ...
        setstatus_modal_btn = disnake.ui.Button(label="Enable/Disable status",
                                                emoji='🔊', style=disnake.ButtonStyle.grey)
        setstatus_modal_btn.callback = self.set_status_modal
        self.add_item(setstatus_modal_btn)

        setstatus_perm_btn = disnake.ui.Button(label="Enable/Disable status (permanent)",
                                               emoji='💾', style=disnake.ButtonStyle.grey)
    ...
    def build_embeds(self):
        ...
        embeds.append(
            disnake.Embed(
                description="### Set automatic status in voice or stage channel\n"
    ...
    async def modal_handler(self, inter: disnake.ModalInteraction, values: dict = None):
        ...
        if inter.data.custom_id == "status_voice_channel_perm":
            ...
            if not guild:
                await inter.send("**There are no bots available in the server. Add at least one by clicking the button below.**",
            ...
            if not inter.author.guild_permissions.manage_guild:
                await inter.send("**You don't have the Manage Server permission to change the voice channel status**", ephemeral=True)
                return
            ...
            for b in self.bot.pool.get_guild_bots(inter.guild_id):
                ...
                p.set_command_log(
                    text=f"{inter.author.mention} " + ("enabled" if values["status_voice_value"] else "disabled") + " the automatic status",
                    emoji="📢",
                )
                p.update = True
            ...
            if not player:
                await inter.send("**I'm not playing music in a voice/stage channel...**", ephemeral=True)
                return
    ...
    async def interaction_check(self, inter: disnake.MessageInteraction) -> bool:
        if inter.author.id != self.ctx.author.id:
            await inter.send(f"Only the member {self.ctx.author.mention} can interact in this message.",
                             ephemeral=True)
            return False
        return True
...
class SkinEditorMenu(disnake.ui.View):
        ...
        if self.mode == "select":
            ...
            if skins_opts:
                skin_select = disnake.ui.Select(min_values=1, max_values=1, options=skins_opts,
                                                placeholder="Normal player mode skins")
            ...
            setup_queue_btn = disnake.ui.Button(label="Configure queue placeholder", emoji="<:music_queue:703761160679194734>")
            ...
            back_btn = disnake.ui.Button(label="Back to previous menu", emoji="⬅️")
            back_btn.callback = self.back
            self.add_item(back_btn)

            self.add_item(disnake.ui.Button(label="Placeholder list", emoji="<:help:947781412017279016>", custom_id="skin_editor_placeholders"))
    ...
    async def edit_embed_button(self, inter: disnake.MessageInteraction):
        ...
        await inter.response.send_modal(
            ViewModal(
                view=self, title="Edit the main embed fields", custom_id="skin_editor_edit_embed",
                components=[
                    disnake.ui.TextInput(
                        style=disnake.TextInputStyle.short,
                        label="Embed title:",
    ...
    async def edit_embed_field_button(self, inter: disnake.MessageInteraction):
        ...
        await inter.response.send_modal(
            ViewModal(
                view=self, title="Edit embed field", custom_id="skin_editor_edit_field",
                components=[
                    disnake.ui.TextInput(
                        style=disnake.TextInputStyle.short,
                        label="Field name:",
    ...
    async def setup_queue(self, inter: disnake.MessageInteraction):
        ...
        await inter.response.send_modal(
            ViewModal(
                view=self, title="Queue song list placeholder", custom_id="skin_editor_setup_queue",
                components=[
                    disnake.ui.TextInput(
                        style=disnake.TextInputStyle.short,
                        label="Text format for song names:",
    ...
    async def modal_handler(self, inter: disnake.ModalInteraction):
        ...
        if inter.custom_id == "skin_editor_message_content":
            ...
            await view.inter.edit_original_message("**The skin was saved/edited successfully!**\n"
...
class SelectBotVoice(disnake.ui.View):
    ...
    def build_interactions(self, freebots: List[BotCore] = None):
        ...
        refresh_btn = disnake.ui.Button(label="Refresh list", emoji="🔄", style=disnake.ButtonStyle.blurple)
    ...
    async def bot_select_callback(self, inter: disnake.MessageInteraction):
        ...
        try:
            ...
            await inter.send(f"<@{bot_id}> is no longer present in the server...", ephemeral=True)
        ...
        if not guild:
            await inter.send(f"{bot.user.mention} is no longer in the server...", ephemeral=True)
            await self.update_message()
            return
        ...
        if player:
            ...
            if inter.author.id not in vc.voice_states:
                await inter.send(f"{bot.user.mention} is already in use in the channel {vc.mention}", ephemeral=True)
                await self.update_message()
                return
