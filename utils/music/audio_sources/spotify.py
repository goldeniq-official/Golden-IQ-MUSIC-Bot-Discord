    ...
    async def get_tracks(self, bot: BotCore, requester: int, query: str, search: bool = True, check_title: float = None):
        ...
        if url_type == "album":
            ...
            if result["tracks"] is None:
                raise GenericError("**No results found for the provided album link...**")
            ...
            if result["tracks"]["items"] is None:
                raise GenericError("**No results found for the provided playlist link...**")
        ...
        if not tracks_data:
            raise GenericError("**No results found for the provided Spotify link...**")
