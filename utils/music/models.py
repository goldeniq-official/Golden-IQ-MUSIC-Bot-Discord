...
class LavalinkPlayer(wavelink.Player):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # restrict to DJs and staff only
        ...
        # enable/disable controller mode (for use in skins only)
        ...
        self.initial_hints = [
            f"You can change the player skin/appearance using the command /change_skin or {self.prefix_info}skin "
            f"(Only members with server management permission can use this command).",

            "Give the music player on the server a new look by creating custom skins. Try it using "
            f"the command /custom_skin or {self.prefix_info}customskin (Only members with administrator "
            "permission can use this command).",

            "You can set the automatic voice channel status with information about "
            "the song currently playing. Try using the command /set_voice_status or "
            f"{self.prefix_info}stageannounce (Only members with server management permission can use this feature).",

            f"If the song has lag, audio stuttering, etc. Try changing the voice channel region "
            f"(if you are in a stage channel use the command {self.prefix_info}stageregion)."
        ]
        ...
        if self.bot.config["LASTFM_KEY"] and self.bot.config["LASTFM_SECRET"]:
            self.initial_hints.append(
                f"You can link a last.fm account to record the songs you listen to here on your "
                f"played songs list. Try using the command /lastfm or {self.prefix_info}lastfm."
            )

        if hint_platforms:
            self.initial_hints.append(
                "You can add/integrate profile/channel links from " + " and ".join(hint_platforms) + " to play "
                f"public playlists from the channel/profile via the command {self.prefix_info}play (without including "
                "name/link) or the command /play (via autocomplete search). Try it using "
                f"the command /fav_manager or {self.prefix_info}favmanager by selecting the \"integrations\" option."
            )
    ...
    def __repr__(self):
        return f"<volume={self.volume} " \
               f"current_position={time_format(self.position) if self.position else 'Idle'} " \
        ...
        while True:
            ...
            if not self._new_node_task:
                ...
                try:
                    ...
                    self.set_command_log(f"A failure occurred while reconnecting the player to the voice channel: {e}.", controller=True)
                    self.update = True
                    ...
                    if (ping := round(self.bot.latency * 1000)) > 250:
                        voice_msg = f"Reconnected to the voice channel due to a possible connection instability (ping: {ping}ms)."
                    ...
                    if not voice_msg:
                        self.set_command_log(text=f"The player was terminated due to a lost connection in the channel <#{vc.id}>.",
                    ...
                    if ignore_error_log:=event.cause.startswith((
                        ...
                        if not getattr(self, "yt_warn", None):
                            txt = f"Due to YouTube restrictions on the server `{self.node.identifier}`. During the current session " \
                                  ...
                                  "of the YouTube songs in the queue (the song played may be different from expected " \
                                  "or even skipped if no results are returned)."
                            try:
    ...
    async def channel_cleanup(self):
        ...
        try:
            ...
            print(f"Failed to clear messages in the channel {self.text_channel} [ID: {self.text_channel.id}]:\n"
    ...
    def setup_hints(self):
        ...
        if self.static:
            hints.append("It's possible to pin songs/playlists on the player message when in idle/waiting mode "
                         "to allow members to listen to them publicly. To do this, use the "
                         f"command /fav_manager or {self.prefix_info}favmanager (only members with "
                         "manage server permission can use this feature).")

        elif self.bot.intents.message_content and self.controller_mode:
            hints.append("By creating a conversation/thread on the player message, song-request mode will be "
                         "activated in it (allowing songs to be requested just by sending the name/link of the song in the conversation).")
        ...
        if len(self.bot.pool.get_guild_bots(self.guild.id)) > 1:
            ...
            if bots_in_guild:
                hints.append(
                    "If any member wants to use me in another voice channel without waiting for me to be "
                    f"disconnected or interrupted from the current channel, there are {bots_in_guild} more bot{'s'[:bots_in_guild^1]} on the server that "
                    f"work{'s'[:bots_in_guild^1]} with my system/commands (using the same prefix/slash commands). "
                    f"Try joining a different voice channel and use the command "
                ...
                hints.append(
                    "If any member wants to use me in another voice channel without waiting for me to be "
                    f"disconnected or interrupted from the current channel. You can add {bots_outside_guild} more extra bot{'s'[:bots_outside_guild^1]} "
                    f"to this server that work(s) with the same system/commands (using the same "
                    f"prefix/slash commands). Use the command {self.prefix_info}invite or /invite to add them."
                )

        if self.controller_mode:
            hints.append(
                "By clicking the 🎛️ emoji in some command messages you will be redirected to the player controller."
            )
    ...
    async def members_timeout(self, check: bool, force: bool = False, idle_timeout = None):
        ...
        if check:
            ...
            if self.current and self.auto_pause:
                ...
                try:
                    self.set_command_log(emoji="🔋", text="The **[resource saving]** mode was disabled.", controller=True)
                    if self.current:
        ...
        if self.keep_connected:
            ...
            self.set_command_log(
                emoji="🪫",
                text="The player is in **[resource saving]** mode (this mode will be automatically disabled when "
            ...
            msg = "The player was shut down due to no members in the channel" + (f" <#{vc.id}>" if vc else '') + "..."
                ...
                if track_data.info["sourceName"] == "spotify" and "spotify" not in self.node.info["sourceManagers"] and self.bot.spotify:
                    ...
                    for i in range(3):
                        try:
                            ...
                            self.set_command_log(emoji="⚠️", text=f"Failed to get Spotify recommended songs, attempt {i+1} of 3.", controller=True)
                            self.update = True
                            traceback.print_exc()
                            exception = e
                            await asyncio.sleep(5)
                ...
                if exception:
                    if isinstance(exception, wavelink.TrackLoadError):
                        error_msg = f"**Cause:** ```java\n{exception.cause}```\n" \
                                    f"**Message:** `\n{exception.message}`\n" \
                                    f"**Level:** `{exception.severity}`\n" \
                                    f"**Music server:** `{self.node.identifier}`"
                    else:
                        error_msg = f"**Details:** ```py\n{repr(exception)}```"
                else:
                    error_msg = "There were no results related to the songs played..."

                try:
                    embed = disnake.Embed(
                        description=f"**Failed to get autoplay data:**\n"
                                    f"{error_msg}",
                        color=disnake.Colour.red())
                    await self.text_channel.send(embed=embed, delete_after=10)
                except:
    ...
    async def _process_next(self, start_position: Union[int, float] = 0, inter: disnake.MessageInteraction = None,
        ...
        try:
            ...
            # TODO: revisit this part if a function to activate track loops for queued songs is added
    ...
    async def process_idle_message(self):
        ...
        if not self.keep_connected:
            embed.description += "\n\n**Note:** `The Player will be automatically shut down` " \
    ...
    async def idling_mode(self):
        ...
        msg = "💤 **⠂The player was shut down due to inactivity...**"
        ...
        if isinstance(self.guild.me.voice.channel, disnake.StageChannel):
            ...
            try:
                ...
                if isinstance(e, disnake.Forbidden):
                    self.stage_title_event = False
                    self.set_command_log(emoji="❌", text="Automatic status was disabled due to lack of permission to change status.", controller=True)
                    self.update = True
                print(traceback.format_exc())
    ...
    async def invoke_np(self, force=False, interaction=None, rpc_update=False):
        ...
        if not self.controller_mode:
            ...
            if data.get("components") is None:
                ...
                # and give it a different name (no accents, spaces, or special characters) and modify them as you like.
                # If you want to set a custom skin as default, add/modify the config DEFAULT_SKIN="yourskin"
                    ...
                    disnake.ui.Select(
                        ...
                        options=[
                            ...
                            disnake.SelectOption(
                                ...
                                description="Rewind the current song to the beginning."
                            ),
                            disnake.SelectOption(
                ...
                if isinstance(self.last_channel, disnake.VoiceChannel):
                    data["components"][5].options.append(
                        disnake.SelectOption(
                            ...
                            description="Configure the automatic voice channel status."
                        )
                    )
                    ...
                    try:
                        try:
                            ...
                            if isinstance(self.text_channel, disnake.Thread):
                                ...
                                #    await self.text_channel.send("Preventing the topic from auto-archiving...", delete_after=2)
                                ...
                                if channel.parent.permissions_for(self.guild.me).send_messages_in_threads:
                                    try:
                                        await channel.send(
                                            embed=disnake.Embed(
                                                color=self.bot.get_color(self.guild.me),
                                                description="**The song request session in the current conversation has ended.**",
                                            )
                                        )
                                    except:
                                        pass
        ...
        try:
            ...
            if not selected_track:
                try:
                    ...
                    if exceptions:
                        print("Failed to resolve PartialTrack:\n" + "\n".join(repr(e) for e in exceptions))
                    return
            ...
            traceback.print_exc()
            embed = disnake.Embed(
                description=f"**Failed to get PartialTrack information:\n[{track.title}]({track.uri or track.search_uri})** ```py\n{repr(e)}```\n"
                            f"**Music server:** `{self.node.identifier}`",
                color=disnake.Colour.red())
            await self.report_error(embed, track)
    ...
    async def _wait_for_new_node(self, txt: str = None, ignore_node: wavelink.Node = None):
        ...
        try:
            ...
            self.set_command_log(
                txt or "No music servers are available. I will make a few attempts to connect to a new music server.",
            ...
            while True:
                ...
                if not self.guild.me.voice:
                    try:
                        ...
                        print(traceback.format_exc())
                        self.set_command_log(f"The player was terminated due to the error: {e}", controller=True)
                        await self.destroy()
                        return
                    await self.connect(self.last_channel.id)
