...
class BotPool:
    ...
    async def start_bot(self, bot: BotCore):
        ...
        try:
            ...
            if error.status == 429 or "429 Too Many Requests" in str(error):
                ...
                print(
                    "Application rate-limited by Discord!\n"
                    "Shutting down/Restarting the process in 5 seconds..."
                )
        ...
        if e:
            ...
            if isinstance(e, disnake.PrivilegedIntentsRequired):
                e = "You have not enabled the Privileged Intents in your application<br>" \
                    "Access the Discord developer portal:<br>" \
                    "https://discord.com/developers/applications/<br>" \
                    "click on your application, then click the \"bot\" menu<br>" \
                    "and then enable all intents.<br>" \
                    "Example screenshot: https://i.imgur.com/a9c1DHT.png<br>" \
                    "After fixing, restart the application."

                print(("=" * 30) + f"\nFailed to start the bot configured at: {bot.identifier}\n" + e.replace('<br>', '\n') + "\n" + ("=" * 30))

            elif isinstance(e, disnake.LoginFailure) and "Improper token" in str(e):
                e = "An invalid token was used.<br>" \
                    "Check if the token provided is correct<br>" \
            ...
            if retries >= max_retries:
                ...
                try:
                    ...
                    if data["identifier"] != "LOCAL":
                        print(f'⚠️ - Failed to verify the server [{data["identifier"]}], '
                              f'retrying [{retries}/{max_retries}] in {backoff} seconds.')
                    backoff += 2
                    retries += 1
                ...
                if tokens:=mongo_data.get("refresh_tokens"):
                    for v in tokens.values():
                        try:
                            ...
                            print(f"🌋 - Failed to apply Youtube refreshToken to the lavalink server: {data['identifier']} - {repr(e)}")
                            break
                        else:
                            print(f"🌋 - Youtube refreshToken applied to the lavalink server: {data['identifier']}")
                            break
    ...
    def setup(self):
        ...
        for key, value in self.config.items():
            ...
            if key.lower().startswith("lavalink_node_"):
                try:
                    ...
                    print(f"Failed to add node: {key}, error: {repr(e)}")
        ...
        try:
            ...
            print(f"📥 - Commit version: {self.commit}")
        except:
            self.commit = None
            ...
            async def check(inter: disnake.ApplicationCommandInteraction):
                ...
                if not inter.guild_id:
                    ...
                    raise GenericError("This command cannot be used in private messages.\n"
                                     "Use it in a server where a compatible bot has been added.")
                ...
                if not allow_private and not isinstance(inter.guild, disnake.Guild):
                    ...
                    if not available_bot:
                        raise GenericError(
                            "**There are no available bots in the server. Add at least one by clicking the button below.**",
        ...
        if not self.bots:
            ...
            message = "The bot token was not properly configured!"
...
class BotCore(commands.AutoShardedBot):
    ...
    async def sync_app_commands(self, force=False):
        ...
        if current_cmds == synced_cmds:
            if current_cmds:
                print(f"⚠️ - {self.user} - Commands are already synchronized.")
            return
        ...
        try:
            if isinstance(channel.parent, disnake.ForumChannel):
                ...
                if channel.owner_id in (bot.user.id for bot in self.pool.get_guild_bots(channel.guild.id) if bot.bot_ready):
                    ...
                    raise GenericError("**You cannot use prefix commands in the current post...**\n"
                                       "`Use slash commands (/) here.`", self_delete=True)
        except AttributeError:
            pass
    ...
    async def update_appinfo(self):
        ...
        if self.appinfo.bot_public and not self.config.get("SILENT_PUBLICBOT_WARNING"):
            ...
            warn_msg = f"Warning: The bot [{self.user}] (ID: {self.user.id}) was configured in the developer portal " \
                  "as a public bot\n" \
            ...
            if guilds:
                warn_msg += "\n\nCurrently the bot is in servers where the bot owner (or team member) is not present " \
                            f"or does not have permission to manage the server to add the bot itself " \
    ...
    def load_modules(self, module_list: list = None, load_modules_log=False):
        ...
        for c in self.slash_commands:
            if (desc:=len(c.description)) > 100:
                raise Exception(f"The description of the command {c.name} exceeded the allowed character count "
                                f"in Discord (100), current count: {desc}")
