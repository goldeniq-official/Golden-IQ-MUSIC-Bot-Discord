    ...
    async def interaction_check(self, interaction: disnake.MessageInteraction) -> bool:
        ...
        if interaction.author != self.ctx.author:
            await interaction.send(
                f"Only the member {self.ctx.author.mention} can use the buttons on this message...",
                ephemeral=True
            )
            return False
...
def music_source_emoji_url(url: str):
    ...
    return "<:play:734221719774035968>", "Unknown Platform"
...
async def select_bot_pool(inter: Union[CustomContext, disnake.MessageInteraction, disnake.ApplicationCommandInteraction], first=False, return_new=False, edit_original=False):
    ...
    if not bots:
        ...
        if (bcount:=len([b for b in inter.bot.pool.get_guild_bots(inter.guild_id) if b.appinfo and b.appinfo.bot_public])):
            raise GenericError(
                f"**You need to add at least one compatible bot to the server by clicking the button below:**",
                components=[disnake.ui.Button(custom_id="bot_invite", label=f"Add bot{'s'[:bcount^1]}")]
            )
        else:
            raise GenericError("**There are no bots compatible with my commands on this server...**")
