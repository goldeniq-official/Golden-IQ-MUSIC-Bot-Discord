...
class LastFMView(disnake.ui.View):
    ...
    def build_components(self):
        ...
        if self.session_key:
            ...
            btn1 = disnake.ui.Button(label=f"{'Disable' if self.scrobble_enabled else 'Enable'} scrobble/tracking")
    ...
    async def scrobble_enable(self, interaction: disnake.MessageInteraction):
        ...
        if (retry_after := self.cooldown_scrobble_btn.get_bucket(interaction).update_rate_limit()):
            await interaction.send(f"**You will have to wait {(rta := int(retry_after))} second{'s'[:rta ^ 1]} to enable/disable scrobble/tracking.**", ephemeral=True)
            return
    ...
    async def send_authurl_callback(self, interaction: disnake.MessageInteraction):
        ...
        await interaction.send(f"### [Click here](<{self.auth_url}>) to link your last.fm account (on the page, click \"allow\")\n\n"
                               f"`The link expires at` <t:{self.last_timestamp}:R> `(If expired, click the button again).`\n\n"
                               f"`Warning: Do not show the \"click here\" link to anyone or post it in public places, "
                               f"as this link can grant access to your last.fm account`\n\n"
                               "`If you have already authorized the application, you should wait up to 20 seconds for "
                               "the message above to update confirming the process.`",
...
class LastFmCog(commands.Cog):
    ...
    @commands.command(hidden=True, name="lastfm", aliases=["lastfmconnect", "lfm"],
                      description="Link your last.fm account to track music via scrobble.",
    ...
    async def lastfm(self, inter: disnake.ApplicationCommandInteraction):
        ...
        try:
            if not inter.permissions.embed_links:
                raise GenericError(f"**You do not have permission to embed links/attachments in channel <#{inter.channel_id}>**")
        ...
        if lastfm_user:
            ...
            txt = f"> `👤` **⠂User:** [`{name}`](<{lastfm_user['url']}>)\n" \
                  f"> `⏰` **⠂Account created at:** <t:{lastfm_user['registered']['#text']}:f>\n"

            if lastfm_user['country']:
                txt += f"> `🌎` **⠂Country:** `{lastfm_user['country']}`\n"

            if playcount := lastfm_user['playcount']:
                txt += f"> `🔊` **⠂Total tracks played:** [`{int(playcount):,}`](<https://www.last.fm/user/{lastfm_user['name']}/library>)\n"
            ...
            txt += f"\n`Listen to your music in a voice channel using the command` {slashcmd} `to track them in your " \
                    f"last.fm account`\n"
            ...
            try:
                ...
                if top_tracks:
                    ...
                    embed = disnake.Embed(
                        ...
                        color=embed_color).set_author(name=f"Top 3: Tracks you have listened to most (out of {int(lastfm_user['track_count']):,} total):",
                ...
                if top_artists:
                    ...
                    embed = disnake.Embed(
                        ...
                        color=embed_color).set_author(name=f"Top 3: Artists you have listened to most (out of {int(lastfm_user['artist_count']):,} total):",
                ...
                if top_albuns:
                    ...
                    embed = disnake.Embed(
                        ...
                        color=embed_color).set_author(name=f"Top 3: Albums you have listened to most (out of {int(lastfm_user['album_count']):,} total):",
            ...
            embeds = [disnake.Embed(
                description="**Link (or create) an account on [last.fm](<https://www.last.fm/home>) to track "
                            "all the music you listen to here in your last.fm profile to get suggestions for "
                            "music/artists/albums and have an overall statistic of the music you have listened to, as well as "
                            "access to an incredible community on the platform.**",
                color=embed_color
        ...
        if view.session_key:
            ...
            embeds[0].description += f"\n### The account [{view.username}](<https://www.last.fm/user/{view.username}>) was " \
                                 "linked successfully!\n\n`Now when you listen to music in a voice channel it " \
                                "will be automatically tracked in your last.fm account`"
            ...
            embeds[-1].set_footer(
                text="Your account was unlinked successfully!",
        ...
        if not (album := track.album_name) and track.info["sourceName"] in ("youtube", "soundcloud"):
            ...
            if track.ytid and track.author.endswith(" - topic") and not track.author.endswith("Release - topic") and not track.title.startswith(track.author[:-8]):
                ...
                if (fmdata := self.bot.last_fm.cache.get(track_query)) is None:
                    ...
                    if not result:
                        print(f"⚠️ - Last.FM Scrobble - No results for track: {track_query}")
