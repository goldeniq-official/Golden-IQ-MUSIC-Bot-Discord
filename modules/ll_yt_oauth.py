    ...
    async def send_authurl_callback(self, interaction: disnake.MessageInteraction):
        ...
        await interaction.send(embed=disnake.Embed(
            ...
            description=f"**Link to authenticate your Google account:**"
                        f" ```\n{verification_url}``` "
                        "`If you have already authorized the application, you should wait up to 15 seconds for "
...
class YtOauthLL(commands.Cog):
    ...
    async def oauth_command(self, ctx: CustomContext):
        ...
        embed = disnake.Embed(
            color=color,
            description=f"## Obtain a refresh token for a Google account\n\n"
                        f"⚠️ **Caution!** Use (or create) a throwaway account instead of proceeding with a personal "
                        f"account due to the high chance of the account being banned by Google (avoid using accounts that may contain a phone number or recovery email; if creating one now, avoid adding a phone number and recovery email)."
        )
        ...
        if not (refresh_token:=view.data.get('refresh_token')):
            raise GenericError("**Time to link Google account has expired!**")
        ...
        embed = disnake.Embed(
            ...
            description=f"## Account confirmation:\n"
                        ...
                        f"**Name:** ```ansi\n[31;1m{name}[0m``` "
                        "⚠️ Caution! If this account is for personal use, click the \"No\" button and use (or create) a throwaway account!"
