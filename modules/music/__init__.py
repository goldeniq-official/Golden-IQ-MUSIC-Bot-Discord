...
class Music(PlayerControllerMixin, commands.Cog):
    ...
    @pool_command(
        ...
        description="Enable the automatic channel status/announcement system with the song name.",
    ...
    async def set_voice_status(
        ...
        if not template:
            ...
            if isinstance(inter, CustomContext):
                await inter.send("**Automatic status set successfully!**")
            else:
                await inter.edit_original_message("**Automatic status set successfully!**")
    ...
    async def message_play(self, inter: disnake.MessageCommandInteraction):
        ...
        if not inter.target.content:
            emb = disnake.Embed(description=f"There's no text in the selected [message]({inter.target.jump_url})...",
    ...
    async def search(
            ...
            query: str = commands.Param(name="busca", desc="Song name or link."),
            *,
            position: int = commands.Param(name="position", description="Place the song at a specific position",
                                           default=0),
    ...
    async def do_connect(
        ...
        try:
            ...
            raise GenericError(
                f"**The bot {bot.user.mention} player was terminated before connecting to the voice channel "
        ...
        if deafen_check and bot.config["GUILD_DEAFEN_WARN"]:
            ...
            if not await check_deafen(me):
                await text_channel.send(
                    embed=disnake.Embed(
                        title="Notice:",
                        description="To protect your privacy and help me save "
                                    "resources, I recommend disabling my audio in the channel by right-clicking "
                                    "on me and then checking: disable "
                                    "audio on server.",
        ...
        if isinstance(channel, disnake.StageChannel):
            ...
            if stage_perms.mute_members:
                ...
                embed.set_footer(
                    text="💡 Tip: to allow me to speak on stage automatically, you need to grant me "
                         "the mute members permission (on the server or just in the chosen stage channel).")
    ...
    stage_flags.add_argument('-position', '-pos', '-p', type=int, default=0, help='Place the song at a specific position in the queue (ignored if -next etc is used).\nEx: -p 10')
    ...
    stage_flags.add_argument('-select', '-s', action='store_true', help='Choose a song from the results found.')
    stage_flags.add_argument('-mix', '-rec', '-recommended', action="store_true", help="Add/play recommended songs based on the artist - song name provided.")
    stage_flags.add_argument('-force', '-now', '-n', '-f', action='store_true', help='Play the added song immediately (only effective if a song is currently playing.)')
    ...
    async def play_file(
            ...
            file: disnake.Attachment = commands.Param(
                name="arquivo", description="audio file to play or add to the queue"
            ),
            position: int = commands.Param(name="position", description="Place the song at a specific position",
                                           default=0),
            force_play: str = commands.Param(
    ...
    async def play(
        ...
        """if not inter.author.voice:
            ...
            if not (c for c in guild.channels if c.permissions_for(inter.author).connect):
                raise GenericError(f"**You are not connected to a voice channel, and there are no voice/stage channels "
            ...
            msg = await func(
                embed=disnake.Embed(
                    description=f"**{inter.author.mention} join a voice channel to play your music.**\n"
                                f"**If you do not connect to a channel within 25 seconds, this operation will be cancelled.**",
                    color=color
                )
            )
            ...
            try:
                ...
                await func(
                    embed=disnake.Embed(
                        description=f"**{inter.author.mention} operation cancelled.**\n"
        ...
        if guild.me.voice and bot.user.id not in author.voice.channel.voice_states:
            ...
            if str(inter.channel.id) == guild_data['player_controller']['channel']:
                ...
                if len(free_bots) > 1 and manual_bot_choice == "yes":
                    ...
                    newmsg = await func(
                        embed=disnake.Embed(
                            description=f"**Choose which bot you want to use in the channel {author.voice.channel.mention}**",
                    ...
                    if v.status is None:
                        try:
                            ...
                            func = inter.edit_original_message
                        try:
                            await func(embed=disnake.Embed(description="### Timed out...", color=self.bot.get_color(guild.me)), view=None)
                        except:
                            traceback.print_exc()
                        return
                    ...
                    if v.status is False:
                        try:
                            ...
                            func = inter.edit_original_message
                        await func(embed=disnake.Embed(description="### Operation cancelled.",
        ...
        if not query:
            ...
            if user_data["fav_links"]:
                ...
                txt += f"`Play a song or playlist you liked or added to your favorites.`\n"
            ...
            if user_data["integration_links"]:
                ...
                txt += f"`You haven't added any integration... " \
                        f"Use integrations to add YouTube channel links (or user profile links from music platforms) for easy access to all their public playlists.`\n"
            ...
            if os.path.isfile(f"./local_database/saved_queues_v1/users/{inter.author.id}.pkl"):
                txt += f"`Use the music queue you saved via command` {savequeue_slashcmd}.\n"
                opts.append(disnake.SelectOption(label="Use saved queue", value=">> [💾 Fila Salva 💾] <<", emoji="💾"))
            ...
            if user_data["last_tracks"]:
                txt += "### `[📑] Recent songs [📑]`\n" \
                    "`Play a song you recently listened to/added.`\n"
            ...
            if not select_interaction or view.selected is False:
                ...
                embed.set_footer(text="⚠️ " + ("Selection timed out!" if view.selected is not False else "Cancelled by user."))
        ...
        if query.startswith(">> [💠 Integrations 💠] <<"):
            ...
            if not user_data["last_tracks"]:
                raise GenericError("**You don't have any songs registered in your history...**\n"
            ...
            if not guild_data["player_controller"]["fav_links"]:
                raise GenericError("**The server does not have any pinned links/favorites.**")
            ...
            if len(fav_opts) == 1:
                ...
                if menu == "favs":
                    embed.description = '### `[⭐] ⠂Favorites ⠂[⭐]`\n' \
                                        '`Play a song or playlist you liked or added to your favorites list.`\n' \
                ...
                if not select_interaction or view.selected is False:
                    ...
                    embed.set_footer(text="⚠️ Timed out!" if not view.selected is False else "⚠️ Cancelled by user.")
        ...
        if query.startswith(">> [💾 Fila Salva 💾] <<"):
            ...
            try:
                ...
                raise GenericError("**Your saved queue has already been deleted...**")
            ...
            if urls:
                ...
                if query.startswith("https://www.youtube.com/results"):
                    ...
                    embed = disnake.Embed(
                        description='**The link contains a video with a playlist.**\n'
                                    ...
                                    f'-# Note: If the link is for a private playlist, only the video from the current link will be loaded.',
                        color=self.bot.get_color(guild.me)
                    )
                ...
                embed = disnake.Embed(
                    ...
                    description="**Select the service you want to prioritize for music search?**\n"
                                "-# Note: If the desired song is not found on the chosen service, another will be used automatically.\n"
                                f'-# Note 2: If no option is chosen below <t:{int((disnake.utils.utcnow() + datetime.timedelta(seconds=45)).timestamp())}:R> the default service will be used automatically.'
                )
            ...
            if not queue_loaded and len(tracks) > 1 and manual_selection:
                ...
                try:
                    ...
                    try:
                        ...
                        func = msg.edit
                    try:
                        await func(embed=disnake.Embed(color=disnake.Colour.red(), description="**Timed out!**"), view=None)
    ...
    case_sensitive_args.add_argument('-casesensitive', '-cs', action='store_true',
                             help="Search for songs with the exact phrase in the song name instead of word by word.")
    @check_stage_topic()
    @check_yt_cooldown()
    @is_requester()
    @check_queue_loading()
    ...
    async def back(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if not len(player.queue) and (player.keep_connected or not len(player.played)):
            await player.seek(0)
            await self.interaction_message(inter, "went back to the start of the song.", emoji="⏪")
            return
    ...
    @has_source()
    @check_voice()
    @pool_command(name="volume", description="Adjust the music volume.", aliases=["vol", "v"], only_voiced=True,
    ...
    @has_source()
    @check_voice()
    @pool_command(name="seek", aliases=["sk"], description="Seek/rewind the music to a specific time.",
    ...
    async def seek(
            self,
            inter: disnake.ApplicationCommandInteraction,
            position: str = commands.Param(name="tempo", description="Time to seek/rewind to (ex: 1:45 / 40 / 0:30)")
    ):
        ...
        if seconds is None:
            raise GenericError(
                "**You used an invalid time! Use seconds (1 or 2 digits) or the format (minutes):(seconds)**")
    ...
    @check_voice()
    @pool_command(
        description=f"Select loop mode: current song / queue / disable / amount (using numbers).",
    ...
    async def loop(self, ctx: CustomContext, mode: str = None):
        ...
        if not mode:
            ...
            try:
                ...
                embed.description = "Selection timed out!"
                try:
    ...
    async def loop_mode(
        ...
        if mode == 'off':
            ...
            player.current.info["extra"]["track_loops"] = 0
            emoji = "🔂"
            txt = ["enabled loop for current song.",
                   f"{emoji} **⠂{inter.author.mention} enabled loop for current song.**"]

        else:  # queue
            emoji = "🔁"
            txt = ["enabled queue loop.", f"{emoji} **⠂{inter.author.mention} enabled queue loop.**"]
    ...
    async def loop_amount(
        ...
        txt = [
            f"set the number of repetitions for song "
            f"[`{(fix_characters(player.current.title, 25))}`](<{player.current.uri or player.current.search_uri}>) to **{value}**.",
            f"🔄 **⠂{inter.author.mention} set the number of repetitions for song to [{value}]:**\n"
    ...
    async def remove_legacy(self, ctx: CustomContext, *, flags: str = ""):
        ...
        if not unknown:
            raise GenericError("**You didn't add the song name.**")
    ...
    @pool_command(name="readd", aliases=["readicionar", "rdd"], only_voiced=True, cooldown=queue_manipulation_cd,
                  max_concurrency=remove_mc, description="Re-add previously played songs to the queue.")
    ...
    async def readd_songs(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if not player.played and not player.failed_tracks:
            raise GenericError("**There are no previously played songs.**")
        ...
        txt = [
            f"re-added [{qsize}] previously played song{(s:='s'[:qsize^1])} to the queue.",
    ...
    async def song_request_thread(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if player.static:
            raise GenericError("**You cannot use this command with a song-request channel configured.**")
        ...
        txt = [
            "Activated the temporary thread/conversation system for song requests.",
    ...
    @pool_command(name="nightcore", aliases=["nc"], only_voiced=True, cooldown=nightcore_cd, max_concurrency=nightcore_mc,
                  description="Enable/Disable the nightcore effect (Sped-up music with a higher pitch).")
    ...
    @commands.command(name="nowplaying", aliases=["np", "npl", "current", "tocando", "playing"],
                 description="Display information about the song you are currently listening to.", cooldown=np_cd)
    ...
    async def now_playing(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if not player.current:
            raise GenericError(f"**I'm not currently playing anything in the channel {player.last_channel.mention}**")
        ...
        components = [disnake.ui.Button(custom_id=f"np_{inter.author.id}", label="Refresh", emoji="🔄")]
        ...
        if player.guild_id != inter.guild_id:
            ...
            footer_kw["text"] = f"In server: {player.guild.name} [ ID: {player.guild.id} ]"
            ...
            if player.guild_id == inter.guild_id:
                ...
                txt += f"### 🎶 ⠂Next songs ({(qsize := len(player.queue + player.queue_autoplay))}):\n" + (
                    ...
                    f"> `⏲️ {time_format(t.duration) if not t.is_stream else '🔴 Live'}`" + (
                        f" - `Loops: {t.track_loops}`" if t.track_loops else "") + \
    ...
    @pool_command(name="controller", aliases=["ctl"], only_voiced=True, cooldown=controller_cd,
                  max_concurrency=controller_mc, description="Send the player controller to a specific/current channel.")
    ...
    async def controller(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if player.static:
            raise GenericError("This command cannot be used in the player's fixed mode.")

        if player.has_thread:
            raise GenericError("**This command cannot be used with an active conversation on the "
                               f"player [message]({player.message.jump_url}).**")
        ...
        if not isinstance(inter, CustomContext):
            await inter.edit_original_message("**Player resent successfully!**")
    ...
    async def add_dj(
        ...
        if user.bot:
            ...
            error_text = f"you cannot add the member {user.mention} to the DJ list (they have permission to **manage channels**)."
        elif user.id == player.player_creator:
            error_text = f"**The member {user.mention} is the player creator...**"
        elif user.id in player.dj:
            error_text = f"**The member {user.mention} is already in the DJ list**"
    ...
    async def remove_dj(
        ...
        if user.id == player.player_creator:
            if inter.author.guild_permissions.manage_guild:
                ...
                raise GenericError(f"**The member {user.mention} is the player creator.**")
    ...
    async def save_queue(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if len(tracks) < 3:
            raise GenericError(f"**You need at least 3 songs to save (current and/or in the queue)**")
        ...
        embed = disnake.Embed(
            ...
            description=f"### {inter.author.mention}: The queue was saved successfully!!\n"
                        f"**Songs saved:** `{len(tracks)}`\n"
                        "### How to use?\n"
                        f"* Using the command {slashcmd} (selecting in the search autocomplete)\n"
                        ...
                        "without including a song name or link."
        )

        embed.set_footer(text="Note: This is a very experimental feature, the saved queue may be changed or removed "
                              "in future updates")
    ...
    @pool_command(name="shuffle", aliases=["sf", "shf", "sff", "misturar"], only_voiced=True,
                  description="Shuffle the songs in the queue", cooldown=queue_manipulation_cd, max_concurrency=remove_mc)
    ...
    async def shuffle_(self, inter: disnake.ApplicationCommandInteraction):
        ...
        await self.interaction_message(
            inter,
            ["shuffled the songs in the queue.",
             f"🔀 **⠂{inter.author.mention} shuffled the songs in the queue.**"],
            emoji="🔀"
        )
    ...
    @has_player()
    @check_voice()
    @pool_command(name="queue", aliases=["q", "fila"], description="Display the songs that are in the queue.",
    ...
    adv_queue_flags.add_argument('-songtitle', '-name', '-title', '-songname', nargs='+',
                                 help="Include name that is in the song.\nEx: -name NCS", default=[])
    adv_queue_flags.add_argument('-uploader', '-author', '-artist', nargs='+', default=[],
                                 help="Remove songs with the name in the specified author/artist/uploader.\nEx: -uploader sekai")
    ...
    adv_queue_flags.add_argument('-playlist', '-list', '-pl', nargs='+', default=[],
                                 help="Remove songs that have the specified name in the associated playlist.\nEx: -playlist myplaylist")
    ...
    async def clear(
            ...
            song_author: str = commands.Param(name="uploader",
                                              description="Include a name that is in the song's author/artist/uploader.",
                                              default=None),
            ...
            duplicates: bool = commands.Param(name="duplicates", description="Include duplicate songs",
                                              default=False),
            playlist: str = commands.Param(description="Include name that is in the playlist.", default=None),
            min_duration: str = commands.Param(name="min_duration",
                                               description="Include songs with duration above/equal (e.g. 1:23).",
                                               default=None),
            max_duration: str = commands.Param(name="max_duration",
                                               description="Include songs with the specified maximum duration (e.g. 1:45).",
                                               default=None),
            amount: int = commands.Param(name="amount", description="Number of songs to remove.",
                                         min_value=0, max_value=99, default=None),
            range_start: int = commands.Param(name="start_position",
                                              description="Include songs from the queue starting at the specified position.",
        ...
        if not filters and not range_start and not range_end:
            ...
            if range_start > 0 and range_end > 0:
                ...
                txt.append(f"**Queue start position:** `{range_start}`\n"
                           f"**Queue end position:** `{range_end}`")
                ...
                txt.append(f"**Queue end position:** `{range_end}`")
            else:
            ...
            try:
                ...
                txt.append(f"**Includes name in uploader/artist:** `{fix_characters(song_author)}`")
            except:
                pass
            ...
            try:
                ...
                txt.append(f"**With start/equal duration:** `{time_format(min_duration)}`")
            except:
                pass
    ...
    @pool_command(name="move", aliases=["movequeue", "moveadv", "moveadvanced", "moveq", "mq", "mv", "mover"],
                  description="Move songs in the queue.", only_voiced=True,
    ...
    async def move(
            ...
            song_name: str = commands.Param(name="name", description="Include a name that is in the song.",
                                            default=None),
            position: int = commands.Param(name="position", description="Target position in the queue (Optional).",
    ...
    @pool_command(name="restrictmode", aliases=["rstc", "restrict", "restrito", "modorestrito"], only_voiced=True, cooldown=restrict_cd, max_concurrency=restrict_mc,
                  description="Enable/Disable the restricted command mode that requires DJ/Staff.")
    ...
    async def restrict_mode(self, inter: disnake.ApplicationCommandInteraction):
        ...
        text = [
            f"{msg[0]} the player's restricted command mode (which requires DJ/Staff).",
            f"{msg[1]} **⠂{inter.author.mention} {msg[0]} the player's restricted command mode (which requires DJ/Staff).**"
        ]
    ...
    @pool_command(name="247", aliases=["nonstop"], only_voiced=True, cooldown=nonstop_cd, max_concurrency=nonstop_mc,
                  description="Enable/Disable the 24/7 player mode (In testing).")
    ...
    async def nonstop(self, inter: disnake.ApplicationCommandInteraction):
        ...
        text = [
            f"{msg[0]} the 24/7 (uninterrupted) player mode.",
            f"{msg[1]} **⠂{inter.author.mention} {msg[0]} the 24/7 (uninterrupted) player mode.**"
        ]
    ...
    @pool_command(name="autoplay", aliases=["ap", "aplay"], only_voiced=True, cooldown=autoplay_cd, max_concurrency=autoplay_mc,
                  description="Enable/Disable automatic playback when the queue runs out of songs.")
    ...
    async def autoplay(self, inter: disnake.ApplicationCommandInteraction):
        ...
        text = [f"{msg[0]} automatic playback.",
                f"{msg[1]} **⠂{inter.author.mention} {msg[0]} automatic playback.**"]
    ...
    @commands.command(aliases=["puptime"], description="View information about how long the player has been active in the server.")
    ...
    async def playeruptime(self, ctx: CustomContext):
        ...
        if not uptime_info:
            raise GenericError("**There are no active players in the server.**")
    ...
    async def resume_players_ready(self):
        ...
        for guild_id in list(self.bot.music.players):
            ...
            try:
                ...
                if not vc:
                    print(
                        f"{self.bot.user} - {player.guild.name} [{guild_id}] - Player stopped due to lack of voice channel")
                    try:
                        await player.destroy()
                    except:
                        traceback.print_exc()
                                ...
                                if not isinstance(channel_db.parent, disnake.ForumChannel) or not channel_db.parent.permissions_for(channel_db.guild.me).create_forum_threads:
                                    ...
                                    if not thread:
                                        thread_wmessage = await channel_db.parent.create_thread(
                                            ...
                                            content="Post for song requests.",
                ...
                if channel_db:
                    ...
                    if not send_message_perm:
                        raise GenericError(
                            ...
                            "If you want to reset the song request channel configuration, use the /reset or /setup command "
                            "again..."
                        )
                ...
                if len(multichoice_opts) > 1:
                    ...
                    if not view.inter:
                        await inter.edit_original_message(
                            content=f"{inter.author.mention}, time's up!",
        ...
        if player and isinstance(message.channel, disnake.Thread) and not player.static:
            ...
            if not self.bot.intents.message_content:
                ...
                await message.channel.send(
                    ...
                    embed=disnake.Embed(
                        description="Unfortunately I cannot read the content of your message...\n"
                                    "Try adding a song using **/play** or click one of the buttons below:",
        ...
        try:
            ...
            if not message.content:
                ...
                try:
                    ...
                    await message.channel.send(f"{message.author.mention} you must send a link/song name.", delete_after=8)
                    return
            ...
            if not urls:
                ...
                if "&list=" in message.content:
                    ...
                    view = ButtonInteraction(
                        ...
                        buttons=[
                            disnake.ui.Button(label="Load song only", emoji="🎵", custom_id="music"),
                            disnake.ui.Button(label="Load playlist", emoji="🎶", custom_id="playlist"),
                        ]
                    )
    ...
    async def parse_song_request(self, message: disnake.Message, text_channel, data, *, response=None, attachment: disnake.Attachment=None, source=None):
        ...
        try:
            if message.guild.me.voice.channel != message.author.voice.channel:
                raise GenericError(
                    f"You must join channel <#{message.guild.me.voice.channel.id}> to request a song.")
        except AttributeError:
            pass
    ...
    async def get_tracks(
        ...
        if mix:
            if not self.bot.pool.last_fm:
                raise GenericError("**At the moment there is no support for mix/recommendations because Last.fm has not been configured in my setup.**")
            ...
            if not info:
                ...
                if not info:
                    txt = f"**No mix results found for your search: {artist} - {track}**"
                    if exceptions:
        ...
        if bool(sc_recommended.search(query)):
            try:
                ...
                raise GenericError("**yt-dlp usage is disabled...**")
    ...
    async def thread_song_request(self, thread: disnake.Thread, reopen: bool = False, bot: BotCore = None):
        ...
        if not thread.parent.permissions_for(thread.guild.me).send_messages_in_threads:
            await player.text_channel.send(
                embed=disnake.Embed(
                    ...
                    description="**I do not have permission to send messages in threads of the current channel to activate "
        ...
        if not bot.intents.message_content:
            embed.description = "**Warning! The message_content intent has not been enabled by my developer...\n" \
                                "The music request feature here may not work as expected...**"
            ...
            if reopen:
                embed.description = "**The music request session in this thread has been reopened in the current thread.**"
            else:
                embed.description = "**This conversation will be temporarily used for song requests.**\n\n" \
                                    "**Request your song here by sending its name or a link to a song/video " \
    ...
    async def player_vc_disconnect(
        ...
        if member.id == player.bot.user.id:
                ...
                try:
                    ...
                    if before.channel.permissions_for(member.guild.me).connect:
                        ...
                        player.set_command_log(text="O player foi finalizado porque me moveram ao canal "
                                                    ...
                                                    "was also connected generating incompatibility with "
            ...
            try:
                ...
                # tempfix so the voice_client channel is not set when moving the bot from the channel.
    ...
    async def check_available_bot(self, inter, guild: disnake.Guild, bot: BotCore = None, message: disnake.Message = None):
        ...
        if not free_bots:
            ...
            if bot_count:
                txt = "**All bots are in use at the moment...**"
                if voice_channels:
                    txt += "\n\n**You can join one of the channels below where there are active sessions:**\n" + ", ".join(
                        voice_channels)
                    if inter.author.guild_permissions.manage_guild:
