...
class Owner(commands.Cog):
    ...
    async def getlavaservers(self, ctx: CustomContext):
        ...
        await ctx.send(
            embed=disnake.Embed(
                description="**The lavalink.ini file was downloaded successfully!\n"
                            "A restart will be required to use the servers from this file.**"
            )
        )
    ...
    updatelavalink_flags.add_argument("-resetids", "-reset", "--resetids", "--reset",
                                      help="Reset music track IDs info (useful to avoid issues with certain "
    ...
    async def reloadconfig(self, ctx: Union[CustomContext, disnake.MessageInteraction]):
        ...
        txt = "**The bot configurations have been reloaded successfully!**"
    ...
    async def reload(self, ctx: Union[CustomContext, disnake.MessageInteraction], *modules):
        ...
        if loaded := data["loaded"] + data["reloaded"]:
            txt += f'**Modules loaded/reloaded:** ```ansi\n[0;34m{" [0;37m| [0;34m".join(loaded)}```\n'
    ...
    update_flags.add_argument("-force", "--force", action="store_true",
                              help="Force update ignoring the local repository state).")
    ...
    async def update(self, ctx: Union[CustomContext, disnake.MessageInteraction], *,
                     opts: str = ""):  # TODO: Check if there is a way to use commands.Flag without a mandatory argument, e.g. --pip.
        ...
        if not git_clean:
            ...
            try:
                ...
                if "Already up to date" in pull_log:
                    raise GenericError("**Already up to date...**")
                out_git += pull_log
        ...
        text = "`A restart will be required after the changes.`"
    ...
    async def update_deps(self, ctx, original_reqs, args, use_poetry=False):
        ...
        if args.pip:
            ...
            if original_reqs != requirements_new:
                ...
                await ctx.send(
                    embed=disnake.Embed(
                        description="**Dependencies need to be updated using the command "
                                    "abaixo no terminal/shell:**\n"
                                    f"```sh\n{txt}{cmd}```\nor use the command: "
                                    f"```ansi\n[34;1m{prefix}update --force --pip[0m``` \n"
                                    f"**Note:** Depending on the hosting (or if there is not 150mb of free RAM "
                                    f"and 0.5vCPU) you should upload the requirements.txt file instead of "
    ...
    async def updatelog(self, ctx: Union[CustomContext, disnake.MessageInteraction], amount: int = 10):
        ...
        if not os.path.isdir(os.environ["GIT_DIR"]):
            raise GenericError("No repository initialized in the bot directory...\nNote: Use the update command.")
    ...
    async def setprefix(self, ctx: CustomContext, prefix: str):
        ...
        if not prefix or len(prefix) > 5:
            raise GenericError("**The prefix cannot contain spaces or be more than 5 characters.**")
    ...
    @commands.cooldown(1, 10, commands.BucketType.guild)
    @commands.command(
        description="Reset the server prefix (use the bot default prefix)"
    )
    async def resetprefix(self, ctx: CustomContext):
        ...
        if not guild_data["prefix"]:
            raise GenericError("**There is no prefix configured in the server.**")
    ...
    @commands.command(
        ...
        description="Change your user prefix (the prefix I will respond to you regardless "
    ...
    async def resetuserprefix(self, ctx: CustomContext):
        ...
        if not user_data["custom_prefix"]:
            raise GenericError("**You do not have a prefix configured.**")
        ...
        embed = disnake.Embed(
            description=f"**Your user prefix has been removed successfully.**",
            color=self.bot.get_color(ctx.guild.me)
        )
    ...
    async def setguildprefix(self, ctx: CustomContext, server_id: int, prefix: str = None):
        ...
        if not prefix:
            ...
            embed.description = "**The server prefix for the given ID has been reset successfully.**"
    ...
    async def cleardm(self, ctx: CustomContext, amount: int = 20):
        ...
        if not counter:
            raise GenericError(f"**មិនមានសារណាមួយត្រូវបានលុបទេ / No messages were deleted out of {amount} checked...**")
    ...
    async def shell(self, ctx: CustomContext, *, command: str):
        ...
        try:
            ...
            try:
                ...
                traceback.print_exc()
                raise GenericError(
                    "**An error occurred (check the logs/terminal or enable your DM for the next "
    ...
    @commands.command(description='Initialize a player in the server.', aliases=["spawn", "sp", "spw", "smn"])
    async def summon(self, ctx: CustomContext):

        try:
            ctx.bot.music.players[ctx.guild.id]  # type ignore
            raise GenericError("**There is already a player running in the server.**")
        except KeyError:
            pass
    ...
    @commands.is_owner()
    @commands.command(hidden=True, aliases=["setbotbanner"], description="Change the bot banner using an attachment or direct link to a jpg or gif image.")
    async def setbanner(self, ctx: CustomContext, url: str = ""):
        await self.setavatar.callback(self=self, ctx=ctx, url=url, mode="banner")

    @commands.is_owner()
    @commands.command(hidden=True, aliases=["setbotavatar"], description="Change the bot avatar using an attachment or direct link to a jpg or gif image.")
    async def setavatar(self, ctx: CustomContext, url: str = "", mode="avatar"):
        ...
        await func(f"The new {mode} for the bot {bot.user.mention} is being processed. Please wait...", embed=None, view=None)
        ...
        await func(f"The {avatar_txt} of the bot {bot.user.mention} was changed successfully.", view=None, embed=None)
