...
def check_channel_perm(channel: Union[disnake.StageChannel, disnake.VoiceChannel, disnake.ForumChannel, disnake.TextChannel]):
    ...
    if missing_perms:
        raise GenericError(
            f"**{channel.guild.me.mention} does not have the following required permissions in the channel {channel.mention}** ```ansi\n" +
...
class SkinSelector(disnake.ui.View):
    ...
    def rebuild_selects(self):
        ...
        if not self.global_mode:
            self.embed.title = "Skin Selector (Apply to the selected bot)"
    ...
    def load_buttons(self):
        ...
        player_volume_select = disnake.ui.Select(
            ...
            options=[
                ...
                disnake.SelectOption(label=f"Default volume: {i}", default=i == self.default_player_volume,
                                     description="Note: Above 100% the audio may sound bad.",
        ...
        check_other_bots_button = disnake.ui.Button(label="Don't connect with incompatible bots.",
    ...
    async def on_timeout(self):
        ...
        if isinstance(self.ctx, CustomContext):
            await self.message.edit(
                embed=disnake.Embed(description="**Timed out...**", color=self.bot.get_color()), view=None
            )
        else:
...
class MusicSettings(commands.Cog):
    ...
    @commands.command(
        ...
        description="Create/choose a dedicated channel for music requests and keep the player pinned.",
    ...
    async def setup(
        ...
        embed_archived = disnake.Embed(
            description=f"**This music request channel was reconfigured by member {inter.author.mention}.**",
        ...
        if not target:
            ...
            embeds = [
                disnake.Embed(
                    description="**Select a channel " + ("or click one of the buttons below to create a new music request channel." if guild.me.guild_permissions.manage_channels else "below:") +'**' ,
                    color=color
                ).set_footer(text="You have only 45 seconds to select/click an option.")
            ]

            if not guild.me.guild_permissions.manage_channels:
                embeds.append(
                    disnake.Embed(
                        description=f"The create channel buttons have been disabled because bot **{bot.user.mention}** "
                                    "does not have **manage channels** permission on the server.",
                        color=color
                    )
                )

            disnake.Embed(color=color).set_footer(
                text="Note: If you want to use a forum channel you will have to select one from the channel list "
                     "below (If you don't have one you will need to create a forum channel manually and use it "
            ...
            if inter_message.data.custom_id.startswith("existing_channel_"):
                ...
                if not guild.me.guild_permissions.manage_channels:
                    raise GenericError(f"**The bot {bot.user.mention} does not have permission to manage channels to create a new channel.**")
        ...
        if target == guild.public_updates_channel:
            raise GenericError("**You cannot use a Discord announcements channel.**")
        ...
        if isinstance(target, disnake.ForumChannel):
            ...
            if not target.permissions_for(guild.me).create_forum_threads:
                raise GenericError(f"**{bot.user.mention} does not have permission to post in channel {target.mention}.**")
            ...
            if not inter.response.is_done():
                ...
                await inter.response.send_modal(
                    title="Define a name for the forum post",
                    custom_id=id_,
                    components=[
                        disnake.ui.TextInput(
                            style=disnake.TextInputStyle.short,
                ...
                try:
                    ...
                    try:
                        ...
                        func = msg_select.edit
                    await func(embed=disnake.Embed(description="### Timed out!", color=bot.get_color(guild.me)), view=None)
                    return
            ...
            if not thread:
                ...
                if not target.permissions_for(guild.me).manage_threads:
                    raise GenericError(
                        f"**{bot.user.mention} does not have permission to manage threads in channel {target.mention}.**\n"
                        f"`Note: You can temporarily grant me this permission and after using the command again "
                        f"you can remove this permission.`")

                """if not target.permissions_for(guild.me).create_forum_threads:
                    raise GenericError(
                        f"**{bot.user.mention} does not have permission to post in channel {target.mention}.**")"""
            ...
            if existing_channel and not guild.me.guild_permissions.administrator and not target.permissions_for(guild.me).manage_permissions:
                raise GenericError(f"**{guild.me.mention} does not have administrator permission or "
                                   f"manage permissions for channel {target.mention}** to edit the permissions "
                                   f"required for the music request system to work properly.\n\n"
                                   f"If you do not want to grant administrator permission or edit the permissions of the"
                                   f" channel {target.mention} to allow me to manage channel permissions. You can use the command "
                                   f"without selecting a destination channel.")

            if not target.permissions_for(guild.me).read_messages:
                raise GenericError(f"{bot.user.mention} permission to read messages in channel {target.mention}")
        ...
        msg = f"{inter.author.mention}, the music request system was configured in channel <#{channel.id}> via bot: {bot.user.mention}"
        ...
        embed = disnake.Embed(
            description=f"**{msg}**\n\nNote: If you want to revert this setting, just use the command {reset_txt} or "
    ...
    async def reset(
        ...
        if not guild.me.guild_permissions.manage_threads:
            raise GenericError(f"I do not have **{perms_translations['manage_threads']}** permission on the server.")
        ...
        if not channel or channel.guild.id != inter.guild_id:
            raise GenericError(f"**There are no music request channels configured in bot {bot.user.mention} (or the channel was deleted).**")

        try:
            if isinstance(channel.parent, disnake.ForumChannel):
                await channel.delete(reason=f"{inter.author.id} reset player")
                if channel_inter != channel:
                    await inter.edit_original_message("The post was deleted successfully!", embed=None, components=None)
        ...
        await func(
            embed=disnake.Embed(
                color=self.bot.get_color(guild.me),
                description="**The music request channel has been reset successfully.**"
            ), components=[]
        )
        ...
        try:
            if delete_channel == "sim":
                ...
                await original_message.edit(
                    content=f"Music request channel was reset by member {inter.author.mention}.",
            ...
            traceback.print_exc()
            raise GenericError(
                "**The music request channel was reset from the database but an error occurred in the process:** "
                f"```py\n{repr(e)}```"
            )
    ...
    async def add_dj_role(
        ...
        if str(role.id) in guild_data['djroles']:
            await inter.send(f"The role {role.mention} is already in the DJ list", ephemeral=True)
            return
        ...
        await inter.send(f"The role {role.mention} has been added to the DJ list.", ephemeral=True)
    ...
    async def remove_dj_role(
        ...
        if not guild_data['djroles']:
            ...
            await inter.send("There are no roles in the DJ list.", ephemeral=True)
            return
        ...
        if str(role.id) not in guild_data['djroles']:
            await inter.send(f"The role {role.mention} is not in the DJ list\n\n" + "Roles:\n" +
        ...
        await inter.send(f"The role {role.mention} has been removed from the DJ list.", ephemeral=True)
    ...
    @commands.has_guild_permissions(manage_guild=True)
    @commands.command(description="Change the player skin/appearance.", name="changeskin", aliases=["skin", "skins"],
    ...
    async def change_skin(self, interaction: disnake.ApplicationCommandInteraction):
        ...
        embed = disnake.Embed(
            description="```ansi\n[31;1mNormal Mode:[0m``` " + ", ".join(f"[`[{s}]`]({bot.player_skins[s].preview})" for s in skin_list) + "\n\n" 
                        ...
                        "\n\n`Note: In global mode the skin will be applied globally across all bots.`",
        ...
        if select_view.skin_selected is None:
            await select_view.interaction.response.edit_message(
                view=None,
                embed=disnake.Embed(description="**Request cancelled.**", colour=bot.get_color(guild.me))
            )
            return
        ...
        if not changed_skins_txt:
            txt = "**No changes were made to the skin settings...**"
        else:
            txt = f"**The server player skin was changed successfully.**\n{changed_skins_txt}"
    ...
    @pool_command(aliases=["la"], description="Enable sending an invite to listen along via RPC "
    ...
    async def listenalong(self, ctx: CustomContext):
        ...
        await ctx.reply(
            embed=disnake.Embed(
                description=f"**Create an invite in channel {ctx.author.voice.channel.mention} by checking the option "
                            f"\"Guest invite\" and then click the button below to send the link to "
                            f"invite link.**\n\n"
                            f"Caution! If you don't have this option, the feature is not available on your "
                            f"server and I recommend not proceeding to avoid giving permanent access to the member "
                            f"who uses the button or to avoid permission issues etc."
            ).set_image(url="https://cdn.discordapp.com/attachments/554468640942981147/1108943648508366868/image.png").
            set_footer(text="Note: create an invite without limitations like: expiration dates, number of uses or "
                            "for only one user to use."),
    ...
    async def send_listen_along_invite(self, inter: disnake.MessageInteraction):
        ...
        if not inter.author.voice.channel:
            return await inter.send("**You need to be in a voice channel to send the invite.**", ephemeral=True)

        await inter.response.send_modal(
            title="Invite to listen along",
            custom_id="listen_along_modal",
            components=[
                disnake.ui.TextInput(
                    style=disnake.TextInputStyle.short,
                    label="Paste the invite in the field below:",
    ...
    async def listen_along_modal(self, inter: disnake.ModalInteraction):
        ...
        if retry_after:
            return await inter.send("**You must wait {} to send the invite**".format(time_format(int(retry_after) * 1000, use_names=True)), ephemeral=True)
        ...
        try:
            ...
            return await inter.edit_original_message("Invalid link or the invite does not exist/has expired")

        if invite.max_uses:
            return await inter.edit_original_message("The invite may have reached the maximum number of uses")

        if invite.target_user:
            return await inter.edit_original_message("The invite cannot be set for only 1 user to use.")
        ...
        for bot in self.bot.pool.get_guild_bots(inter.guild_id):
            ...
            if not isinstance(channel, disnake.VoiceChannel):
                return await inter.edit_original_message("**This feature only works in voice channels.**")
        ...
        if len(global_data["listen_along_invites"]) > 4:
            return await inter.edit_original_message(
                embed=disnake.Embed(
                    description="**Invite limit exceeded on the current server, delete at least one of the invites "
        ...
        await inter.edit_original_message(
            f"**The link {invite} was activated/updated successfully to be sent via RPC when there is an "
            f"active player in channel {inter.author.voice.channel.mention}.**\n"
            f"`Note: If you want to display it in your status and don't have the RPC app, use the /rich_presence command for "
            f"more information.`"
        )
    ...
    @commands.command(
        ...
        description="View information about music servers."
    )
    ...
    async def nodeinfo(self, interaction: disnake.ApplicationCommandInteraction):
        ...
        for page in disnake.utils.as_chunks(available_nodes, 6):
            ...
            if failed_nodes:
                em.add_field(name="**Failed servers** `❌`",
    ...
    @commands.command(name="customskin", aliases=["setskin", "cskin", "cs", "ss"],
                      description="Create your own skins/templates to use in the music player.",
        ...
        await inter.send(
            ...
            embed=disnake.Embed(
                ...
                description="### Placeholders for custom skins:\n```ansi\n"
                            ...
                            "[34;1m{track.title_25}[0m -> Song name (up to 25 characters)\n"
                            "[34;1m{track.title_42}[0m -> Song name (up to 42 characters)\n"
                            "[34;1m{track.title_58}[0m -> Song name (up to 58 characters)\n"
                            "[34;1m{track.url}[0m -> Song link\n"
                            "[34;1m{track.author}[0m -> Song uploader/artist name\n"
                            "[34;1m{track.duration}[0m -> Song duration/time\n"
                            "[34;1m{track.thumb}[0m -> Song thumbnail/artwork link\n"
                            "[34;1m{playlist.name}[0m -> Name of the source playlist\n"
                            "[34;1m{playlist.url}[0m -> Link/URL of the source playlist\n"
                            "[34;1m{player.loop.mode}[0m -> Player loop/repeat mode\n"
                            "[34;1m{player.queue.size}[0m -> Number of songs in the queue\n"
                            "[34;1m{player.volume}[0m -> Player volume\n"
                            "[34;1m{player.autoplay}[0m -> Autoplay (Enabled/Disabled)\n"
                            "[34;1m{player.nightcore}[0m -> Nightcore effect (Enabled/Disabled)\n"
                            "[34;1m{player.hint}[0m -> Player usage tips\n"
                            "[34;1m{player.log.text}[0m -> Player log\n"
                            ...
                            "[34;1m{requester.avatar}[0m -> Link to the avatar of the member who requested the song\n"
                            "[34;1m{guild.color}[0m -> Color of the highest bot role on the server\n"
                            "[34;1m{guild.icon}[0m -> Server icon link\n"
                            "[34;1m{guild.name}[0m -> Server name\n"
                            "[34;1m{guild.id}[0m -> Server ID\n"
                            "[34;1m{queue_format}[0m -> Pre-formatted queue songs (use the settings button "
                            "placeholder to change the style)\n"
                            "[34;1m{track.number}[0m -> Song position number in the queue (functional together with "
...
class RPCCog(commands.Cog):
    ...
    async def stageregion(self, ctx: CustomContext):
        ...
        await func(
            embed=disnake.Embed(
                color=color,
                description=f"**The region of channel {vc.mention} was successfully changed to:\n"
                            f"{view.selected}**"
            ), view=None
        )
    ...
    async def rich_presence(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if not self.bot.config["RPC_PUBLIC_URL"] and not self.bot.config["RPC_SERVER"]:
            raise GenericError("**The RPC_SERVER has not been configured in ENV/ENVIRONMENTS (or .env file)**")
        ...
        if self.bot.config["ENABLE_RPC_AUTH"]:
            ...
            embed.description += "\n**You will need to create/generate/import a token to grant RPC access " \
        ...
        embed.description += "\n\n**Now just click the \"Start Presence\" button and listen to music through " \
                             "a compatible bot.**"
    ...
    async def rpc_button_event(self, inter: disnake.MessageInteraction):
        ...
        if user_id != str(inter.author.id):
            await inter.send(f"Only <@{user_id}> can use the buttons of this message!", ephemeral=True)
            return
        ...
        if button_id == "rpc_gen":
            ...
            msg = f"The token for use in the RPC (Rich Presence) app was generated successfully!\n\n" \
            ...
            await inter.response.send_modal(
                ...
                components=[
                    disnake.ui.TextInput(
                        ...
                        placeholder="Note: For security reasons, never include a personal password here!",
            ...
            data["token"] = ""
            await self.bot.update_global_data(id_=user_id, data=data, db_name=DBModel.users)
            msg = "The token was removed successfully!\n" \
                  "The RPC system will now be disabled for your user."
