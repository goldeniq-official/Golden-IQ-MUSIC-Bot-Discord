...
class PlayerSession(commands.Cog):
    ...
    @commands.is_owner()
    @commands.command(hidden=True, description="Save player information to the database immediately.", aliases=["svplayers"])
    ...
    async def voice_check(self, voice_channel: Union[disnake.VoiceChannel, disnake.StageChannel], position: int = 0):
        ...
        if not wait_counter:
            print(f"{self.bot.user} - {guild.name}: Player ignored due to delay connecting to the voice channel.")
            return
        ...
        if isinstance(voice_channel, disnake.StageChannel) and \
            ...
            try:
                ...
                print(f"{self.bot.user} - Failed to speak on stage in server {guild.name}. Error: {repr(e)}")
        ...
        try:
            ...
            try:
                ...
                if text_channel:
                    ...
                    if text_channel:
                        try:
                            ...
                            print(f"{self.bot.user} - Controller Ignored (missing permission) [Channel: {text_channel.name} | ID: {text_channel.id}] - [ {guild.name} - {guild.id} ]")
                            text_channel = None
                        else:
                            if data["message_id"]:
                                try:
                ...
                if not voice_channel:
                    ...
                    try:
                        msg = "Player stopped because the voice channel does not exist or was deleted."
                ...
                try:
                    ...
                    print(f"{self.bot.user} - Player Ignored: {guild.name} [{guild.id}]\n{repr(e)}")
                    try:
                        msg = f"The player was stopped due to missing permission to connect to channel {voice_channel.mention}."
                        if not data["skin_static"]:
                            await text_channel.send(embed=disnake.Embed(description=msg, color=self.bot.get_color(guild.me)))
                        else:
                            await send_idle_embed(text_channel, bot=self.bot, text=msg)
            ...
            if started:
                ...
                try:
                    ...
                    print(f"{self.bot.user} - Failed to play music when resuming player for server {guild.name} [{guild.id}]:\n{traceback.format_exc()}")
                    return

            print(f"▶️ - {self.bot.user} - Player Resumed: {guild.name} [{guild.id}] - Server: {player.node.identifier}")

        except Exception:
            print(f"{self.bot.user} - Critical failure resuming players:\n{traceback.format_exc()}")
